"""
Watercolor with Python
From "Generative Art - Watercolor with Python" YouTube video
...
def deform(shape, iterations, variants):
    for i in range(iterations):
        for j in range(len(shape)-1, 0, -1):
            midpoint = ((shape[j-1][0] + shape[j][0])/2 + float_gen(-variants, variants), (shape[j-1][1] + shape[j][1])/2 + float_gen(-variants, variants))
...
def main():
    ...
    ims.write_to_png('watercolor2.png')
